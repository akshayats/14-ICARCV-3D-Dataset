...
\title{ KTH-3D-TOTAL: A 3D Dataset for Discovering Spatial Structures for Long-Term Autonomous Learning
}

\author{\IEEEauthorblockN{
Akshaya Thippur\IEEEauthorrefmark{1}, 
Rares Ambrus\IEEEauthorrefmark{1}, 
Gaurav Agrawal\IEEEauthorrefmark{2}, 
Adri\`a Gallart del Burgo\IEEEauthorrefmark{1}, 
Janardhan Haryadi Ramesh\IEEEauthorrefmark{2}, 
\\Mayank Kumar Jha\IEEEauthorrefmark{2}, 
Malepati Bala Siva Sai Akhil\IEEEauthorrefmark{2},  
Nishan Bhavanishankar Shetty\IEEEauthorrefmark{2}, 
\\John Folkesson\IEEEauthorrefmark{1}, 
Patric Jensfelt\IEEEauthorrefmark{1}}
\IEEEauthorblockA{\IEEEauthorrefmark{1}CVAP/CAS, KTH Royal Institute of Technology, 
SE-100 44 Stockholm, Sweden.}%\\ Email: see http://www.kth.se/en/csc/forskning/cvap}
\IEEEauthorblockA{\IEEEauthorrefmark{2}M.S. Ramaiah Institute of Technology, V.T.U, 560054 Bangalore, India.}}
...
paper, we present a 3D dataset of 20 office table-tops manually observed and scanned 3 times a 
day as regularly as possible over 19 days (477 scenes) and subsequently, manually annotated with 18 different object 
...
napkins etc.\ configurations of which do not vary over many days, 
...
a corner stone in the foundation for designing efficient representations and inference
...
the same set of select tables, over a possible 3 times a day and over several
...
and many similar but not identical scenes. Several datasets have 
been constructed for various research applications. However, these existing 
...
environments, we present the dataset KTH-3D-TOTAL, which has been composed by scanning and capturing observations of a fixed set of entire 
table-tops, as regularly as possible, in an office environment. For what follows, we define a \textit{scene} as a single observation of a table-top instance at a particular 
...
required regularity in instances and temporal coherence was the main motivation for the construction of this dataset, as currently available datasets 
either are of individual objects or single instances of tables or entire rooms. This required regularity in sampling observations and extent over time is 
...
can also possibly be extracted (e.g.\ Weekdays, Fridays, Weekends). Table-top models can 
...
of table-tops in office environments supporting the building of representation systems for the same.
...
The scenes were recorded as periodically as possible and at time instances belonging to three fixed times in the day: \emph{Morning} (09:00 hrs), \emph{Afternoon} (13:00 
...
The data collection was conducted with minimal disturbance to participants and authors which in effect means: Of the $19$ days$\times3$ times a day$=57$ possible collection instances, only a subset were utilized. Even during these collection instances some tables are left out if the participant was occupied in a meeting, busy with important work etc.
...
\begin{itemize}
	\item scenes collected from 20 unique tables, 3 times a day for 19 days and hence in total, 477 scenes.
	\item each scene manually annotated at an average of 12 objects per scene. These objects belong to a superset of 18 Objects-of-Interest classes and could even be multiple instances of the same object class.
	
	%with 18 possible object classes and an average of 12 object instances, from these classes, annotated per scene, including multiple instances
...
\begin{figure}[t]
\begin{center}
\subfloat[][]{\label{fig:HistOfObjects}\includegraphics[width=0.8\linewidth]{clean-data/HistOfObjects-crop.pdf}}\\
...
\caption{(a) Objects annotated in 3D Long-Term Dataset, sorted in descending order of count of occurrences. X-axis$=$Object Name, Y-axis$=$Occurrence Count. (b) Screenshot of one typical office table scene, along with its annotations in (c).}
\end{center}
\end{figure}
...
\begin{figure*}[bhtp]
\begin{center}
\includegraphics[height=2.5cm]{clean-data/puren/131023} \smallskip
\includegraphics[height=2.5cm]{clean-data/puren/131024} \smallskip
\includegraphics[height=2.5cm]{clean-data/puren/131030} \\
\includegraphics[height=2.5cm]{clean-data/puren/131106} \smallskip
\includegraphics[height=2.5cm]{clean-data/puren/131109} \smallskip
\includegraphics[height=2.5cm]{clean-data/puren/131116} 
\caption{This is the table of a single person, observed during the afternoon over many days: \{L to R -- Row 1: 23 Oct, 24 Oct, 30 Oct; Row 2: 06 Nov, 09 Nov, 16 Nov 2013\}. }
...
table over many days. Notice that between 06 Nov and 16 Nov the setup of the table does 
...
consider the variations between 23 Oct and 30 Oct -- this clearly indicates 
...
worth of variation in between the images of 06 Nov and 09 Nov, however there is a huge 
variation in the appearance of the table-top between 23 Oct and 24 Oct suggesting a most 
likely human activity of cleaning/tidying up and the less likely event of an unauthorized 
meddling. Another aspect we would like to capture is the no change in scenes between 
Friday evenings to Monday mornings on account of the weekend inactivity. These are all 
desirable properties that ought to be captured by the long term autonomous systems.
...
speculations but by looking closely at more regular data over 19 days, stronger claims could be made.
...
cases. Such qualitative models could allow some of the inherent generality in 
...
part). Note how each person has a much less spread in the distribution than 
...
\begin{figure}
\begin{center}
\includegraphics[width=0.8\linewidth]{clean-data/keyboard_mouse_raw-crop}
\includegraphics[width=0.8\linewidth]{clean-data/keyboard_mouse_clusters-crop}
\end{center}
...
the keyboard w.r.t.\ to the monitor. Our intuition that keyboards are 
...
\begin{figure*}
\begin{center}
\includegraphics[width=0.5\linewidth]{clean-data/EntropyMat-crop} \bigbreak
\begin{tabular}{|l||c|c|c|c|c|c|c|c|c|c|c|c|c|}
\hline
& key & mon & mou & mug & lap & pap & boo & bot & jug & not \\ \hline \hline
key & 0.80 & 3.58 & 2.93 & 4.08 & 3.69 & 4.57 & 3.97 & 3.41 & 2.76 & 3.66 \\ \hline
mon & 3.43 & 1.61 & 3.37 & 4.03 & 3.40 & 4.57 & 4.04 & 3.43 & 2.96 & 3.48 \\ \hline
mou & 3.05 & 3.63 & 0.74 & 3.94 & 3.59 & 4.53 & 3.79 & 2.28 & 2.84 & 3.62 \\ \hline
mug & 4.17 & 4.15 & 3.78 & 0.70 & 3.73 & 4.41 & 3.80 & 3.94 & 2.76 & 3.40 \\ \hline
lap & 3.88 & 3.52 & 3.73 & 3.63 & 0.06 & 4.32 & 3.58 & 3.04 & 1.99 & 2.94 \\ \hline
pap & 4.36 & 4.42 & 4.12 & 4.28 & 3.85 & 2.68 & 4.09 & 3.72 & 2.92 & 3.53 \\ \hline
boo & 3.81 & 3.88 & 3.75 & 3.92 & 3.48 & 4.34 & 0.63 & 3.10 & 2.08 & 3.33 \\ \hline
bot & 3.47 & 3.56 & 3.37 & 3.10 & 2.86 & 4.06 & 3.02 & 0.94 & 2.46 & 2.57 \\ \hline
jug & 2.97 & 3.19 & 2.82 & 2.92 & 1.90 & 3.19 & 2.23 & 2.77 & 0.00 & 1.91 \\ \hline
not & 3.57 & 3.82 & 3.52 & 3.55 & 2.98 & 3.94 & 3.35 & 2.51 & 1.73 & 0.08 \\ \hline
...
\begin{figure}[t]
\begin{center}
\subfloat[][]{\label{fig:scatter-monitor-keyboard}\includegraphics[width=0.8\linewidth]{clean-data/monitor_keyboard_clusters-crop}}\quad
\subfloat[][]{\label{fig:scatter-keyboard-mug}\includegraphics[width=0.8\linewidth]{clean-data/keyboard_mug_clusters-crop}}\\
\subfloat[][]{\label{fig:scatter-keyboard-papers}\includegraphics[width=0.8\linewidth]{clean-data/keyboard_papers_clusters-crop}}
\end{center}
\caption{The figures show the relative position of a) keyboard w.r.t. monitor, b) mug w.r.t. keyboard and c) papers w.r.t. keyboard. Qualitatively keyboards are mostly in front of monitors, mugs are around the keyboard and the position of papers is mostly independent on the position of the keyboard. The different colors/markers show data from different people in the data.} 
...
relations, such as the mouse is to the \textit{right of} the keyboard and 
the keyboard is \textit{in front} of the monitor, while 
keeping in mind that this represents the typical case and not the only 
possible situation. It is important to differentiate between typical knowledge, 
...
time. We can also see that a system that observes these table-tops for 
...
We have shown that there is structure in many aspects of the manner that objects are
...
providing the means to train long-term autonomous learning systems for spatial 
understanding. We aim to extend this dataset with scenes pertaining to observations 
with more temporal regularity. We will also create datasets of room level organisations of 
objects and even different kinds of table-top scenes (viz.~cafeteria, medical, convenience 
store table-tops) to explore the aspects of generalisability with respect to concept and 
specificity in the purview of long term autonomous robotic learning.
...
allow for activity recognition, anomaly detection, and context recognition.
